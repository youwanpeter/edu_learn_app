...
class _CourseDetailViewState extends State<CourseDetailView> {
  ...
  List<Widget> _buildAppBarActions(CourseViewModel courseViewModel) {
    ...
    final canEdit = widget.user.isStaff && widget.user.id == course.instructorId;
              ...
              ...lessonViewModel.lessons.take(3).map((lesson) {
                return Padding(
                  ...
                  child: LessonItem(
                    ...
                    onEdit: widget.user.isStaff
                        ? () => _navigateToEditLesson(lesson)
                        : null,
                    onDelete: widget.user.isStaff
  ...
  Widget? _buildFloatingActionButton(CourseViewModel courseViewModel) {
    ...
    final canAddLesson = widget.user.isStaff && widget.user.id == course.instructorId;
  ...
  void _deleteLesson(BuildContext context, String lessonId) async {
    ...
    if (confirmed == true) {
      try {
        await context.read<LessonViewModel>().deleteLesson(lessonId, widget.user);
  ...
  // FIXED: Added studentId parameter
  void _markAsComplete(BuildContext context, String lessonId) async {
    try {
      // Pass both lessonId and studentId
      final success = await context.read<LessonViewModel>().markAsCompleted(lessonId, widget.user.id);
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
                      ...
                      Expanded(
                        child: ElevatedButton(
                          ...
                          onPressed: () {
                            ...
                            context.read<LessonViewModel>().markAsCompleted(lesson.id, user.id);
                          },
                          child: const Text(
                            'Mark as Complete',
                            style: TextStyle(fontSize: 16),
