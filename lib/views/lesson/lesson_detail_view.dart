...
class _LessonDetailViewState extends State<LessonDetailView> {
  ...
  List<Widget> _buildAppBarActions(LessonViewModel viewModel) {
    ...
    final canEdit = widget.user.isStaff;
  ...
  void _deleteLesson(BuildContext context, String lessonId) async {
    ...
    if (confirmed == true) {
      try {
        final success = await context.read<LessonViewModel>().deleteLesson(lessonId, widget.user);
        if (success) {
  ...
  void _toggleLock(BuildContext context, String lessonId) async {
    try {
      final success = await context.read<LessonViewModel>().toggleLock(lessonId, widget.user);
      if (success) {
